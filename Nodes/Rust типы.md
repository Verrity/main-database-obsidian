...
#### Sized
`?Sized`
Обычно каждый тип в Rust обязан иметь известный размер на этапе компиляции (например, `i32` — 4 байта, `Vec<T>` — 3 слова). Но есть типы, размер которых неизвестен: например, срез `[i32]` — у него нет конкретного размера, он может быть длиной 1, 5, 100. Такие типы называются **динамически-размерными** (DST).

`?Sized` означает: **"этот тип может быть не sized (не иметь известного размера)"**. Без `?Sized` компилятор бы запретил использовать `[i32]` как параметр `B`.
```rust unfold
